...
namespace SystemFramework.Common
{
    ...
    public class ObjectCollectionView<T> : AggregateObjectColletionView<T>
    {
        /// <summary>
        ...
        public ObjectCollectionView(IList list)
            : base()
        {
            DataSource = list;
        }

        public ObjectCollectionView(IContainer container)
            : base(container)
        {
            DataSource = null;
        }
            ...
            set
            {
                ...
                if (!(value is ICollection<T>))
                {
                    ...
                    foreach (object item in value)
                    {
                        if (!(item is T))
                        {
                           throw new ArgumentException(string.Format(SystemFramework.Properties.Resources.InvalidListItemType, typeof(T).FullName), "DataSource");
                        }
                    }
                }
